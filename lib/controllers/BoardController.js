...
module.exports.initialize = function () {
	...
	express.post(routes.create, function(req,res){
		var id = randomString.generate(6).toLowerCase();


		boards.c

		console.log(users);
		console.log(users[req.cookies.__pulsar__]);
		if(users[req.cookies.__pulsar__])
			boardConfigs[id].instagram = {
				accessToken : users[req.cookies.__pulsar__],
				tags : []
			}
		res.end(id);
	});
